...
export class AuthService {
  ...
  private async setUserLogin(req: Request, res: Response, user: I_User) {
    ...
    const { access_token, refresh_token } =
      await this.tokenService.createLoginCredentials(user._id!, user.role);
  ...
  async refreshToken(
    ...
    const signature =
      refresh_token.split(' ')[0] === SignatureLevelEnum.BEARER
        ? SignatureLevelEnum.BEARER
        : SignatureLevelEnum.SYSTEM;

    const access_token = await this.tokenService.createRefreshToken(
    ...
    await this.tokenService.saveJwt(
      user._id as Types.ObjectId,
      access_token.jti,
      access_token.token,
      TokenTypeEnum.ACCESS,
  ...
  async getSessions(userId: Types.ObjectId, currentJti: string) {
    ...
    const currentSession =
      sessions.find((session) => session.jti === currentJti) || null;
