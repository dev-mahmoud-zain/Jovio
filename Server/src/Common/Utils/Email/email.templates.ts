...
export const changeEmailTemplate = async ({ OTPCode }: { OTPCode: string }): Promise<string> => {

    return ` 
  <div style="font-family: 'Segoe UI', Arial, sans-serif; padding: 30px; background-color: #f4f7fb; color: #333; max-width: 600px; margin: auto; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); text-align: center;">
    
    <h2 style="color: #2c3e50; margin-bottom: 10px;">📧 Change Email Address</h2>
    <p style="font-size: 16px; line-height: 1.6; margin: 0 0 15px;">
        We received a request to change the email address associated with your
        <strong style="color:#4a90e2;">Jovio</strong> account.
    </p>
    <p style="font-size: 15px; line-height: 1.6; margin: 0 0 20px;">
        Please use the following verification code to confirm your new email:
    </p>
    ...
    <p style="font-size: 14px; color: #555; line-height: 1.6; margin: 20px 0;">
        This code is valid for <strong>10 minutes</strong>.<br>
        If you didn’t request to change your email, please ignore this email or secure your account.
    </p>
